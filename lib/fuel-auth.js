...
module.exports = class FuelAuth {
	constructor(options) {	
		var isOauth2Flow = options.authOptions && options.authOptions === 2;
		...
		if (options) {
			...
			if(isOauth2Flow && !options.authOptions.applicationType == 'public'){
				if (!options.clientSecret) {
					throw new Error('clientSecret is missing or invalid');
				}
				if (typeof options.clientSecret !== 'string') {
					throw new Error('clientSecret must be string');
				}
			}
			if (isOauth2Flow && !options.authUrl) {
