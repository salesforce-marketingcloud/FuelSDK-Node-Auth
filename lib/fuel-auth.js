...
module.exports = class FuelAuth {
	constructor(options) {
		if (options) {
			...
			if(options.apiVersion === 2 && !options.tenant){
		...
		this.authUrl = options.authUrl || (options.apiVersion === 2 ? `https://${options.tenant}.auth.marketingcloudapis.com/v2/token` : 'https://auth.exacttargetapis.com/v1/requestToken');
