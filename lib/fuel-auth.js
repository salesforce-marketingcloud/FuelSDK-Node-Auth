...
module.exports = class FuelAuth {
	constructor(options) {
		if (options) {
			...
			if(options.authVersion === 2 && !options.tenant){
				throw new Error('Version 2 APIs require that you specify a tenant');
			}
		} else {
		...
		this.authUrl = options.authUrl || (options.authVersion === 2 ? `https://${options.tenant}.auth.marketingcloudapis.com/v2/token` : 'https://auth.exacttargetapis.com/v1/requestToken');
		...
		return new Promise((resolve, reject) => {
			request(options, (err, res, body) => {
				...
				if(this.authVersion === 2){
					...
					this.expiration = body.expires_in ? process.hrtime()[0] + body.expiresIn : null;
				} else {
